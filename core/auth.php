...
function is_superadmin(): bool { $u=current_user(); return ($u['role_key']??'')==='superadmin'; }
function can(string $perm): bool { $u=current_user(); if(!$u) return false; if(($u['role_key']??'')==='superadmin') return true; $st=db()->prepare('SELECT 1 FROM role_permissions rp JOIN permissions p ON p.id=rp.permission_id WHERE rp.role_id=? AND p.permission_key=?'); $st->execute([(int)$u['role_id'],$perm]); return (bool)$st->fetchColumn(); }
