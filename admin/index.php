...
elseif($page==='api_integrations'){
 ...
 $conns=table_exists('api_connections')?all("SELECT * FROM api_connections WHERE status NOT IN ('revoked','deleted') ORDER BY FIELD(status,'active','pending'), id DESC LIMIT 80"):[];
 ...
 $incoming=table_exists('api_pairing_requests')?all("SELECT * FROM api_pairing_requests WHERE direction='incoming' AND status NOT IN ('cancelled','revoked','deleted') ORDER BY id DESC LIMIT 50"):[];
 $outgoing=table_exists('api_pairing_requests')?all("SELECT * FROM api_pairing_requests WHERE direction='outgoing' AND status NOT IN ('cancelled','revoked','deleted') ORDER BY id DESC LIMIT 50"):[];
