...
elseif($page==='finished'){
 ...
 $q=trim((string)($_GET['q']??'')); $sourceFilter=(string)($_GET['source']??''); $stockFilter=(string)($_GET['stock']??'');
 $perPage=max(25,min(100,(int)($_GET['per_page']??50))); $curPage=max(1,(int)($_GET['p']??1)); $offset=($curPage-1)*$perPage;
 $where=['fp.is_active=1']; $params=[];
 if($q!==''){ $where[]='(fp.name LIKE ? OR fp.sku LIKE ? OR fp.code LIKE ? OR fp.category LIKE ? OR s.store_name LIKE ?)'; $like='%'.$q.'%'; array_push($params,$like,$like,$like,$like,$like); }
 if($sourceFilter==='manual'){ $where[]='fp.source_store_id IS NULL'; }
 elseif(substr($sourceFilter,0,6)==='store:'){ $where[]='fp.source_store_id=?'; $params[]=(int)substr($sourceFilter,6); }
 if($stockFilter==='available') $where[]='COALESCE(st.qty,0)>0';
 elseif($stockFilter==='empty') $where[]='COALESCE(st.qty,0)<=0';
 $baseSql=' FROM finished_products fp LEFT JOIN stores s ON s.id=fp.source_store_id LEFT JOIN (SELECT item_id, COALESCE(SUM(qty_in-qty_out),0) qty FROM stock_ledger WHERE item_type=\'finished\' GROUP BY item_id) st ON st.item_id=fp.id WHERE '.implode(' AND ',$where);
 $totalRows=(int)one('SELECT COUNT(*) c'.$baseSql,$params)['c']; $totalPages=max(1,(int)ceil($totalRows/$perPage)); if($curPage>$totalPages){$curPage=$totalPages; $offset=($curPage-1)*$perPage;}
 $finishedRows=all('SELECT fp.*, s.store_name, COALESCE(st.qty,0) stock_qty'.$baseSql.' ORDER BY fp.name LIMIT '.$perPage.' OFFSET '.$offset,$params);
 $makeUrl=function(array $extra=[]) use($q,$sourceFilter,$stockFilter,$perPage){ $args=array_merge(['page'=>'finished','q'=>$q,'source'=>$sourceFilter,'stock'=>$stockFilter,'per_page'=>$perPage],$extra); return '?'.http_build_query(array_filter($args,fn($v)=>$v!==''&&$v!==null)); };
 echo '<form method="get" class="finished-toolbar card"><input type="hidden" name="page" value="finished"><div class="finished-filter-grid"><p class="finished-search-field"><label>Search<input type="search" name="q" value="'.e($q).'" placeholder="Cari produk / SKU / sumber..."></label></p><p><label>Sumber<select name="source"><option value="">Semua sumber</option><option value="manual"'.($sourceFilter==='manual'?' selected':'').'>Manual</option>'; foreach($activeStores as $s) echo '<option value="store:'.(int)$s['id'].'"'.($sourceFilter==='store:'.(int)$s['id']?' selected':'').'>'.e($s['store_name']).'</option>'; echo '</select></label></p><p><label>Status stok<select name="stock"><option value="">Semua stok</option><option value="available"'.($stockFilter==='available'?' selected':'').'>Stok tersedia</option><option value="empty"'.($stockFilter==='empty'?' selected':'').'>Stok kosong</option></select></label></p><p><label>Per halaman<select name="per_page"><option value="50"'.($perPage===50?' selected':'').'>50</option><option value="100"'.($perPage===100?' selected':'').'>100</option><option value="25"'.($perPage===25?' selected':'').'>25</option></select></label></p><div class="finished-toolbar-actions"><button class="btn light" type="submit">Terapkan</button><a class="btn light" href="?page=finished">Reset</a><button class="btn secondary" type="button" data-finished-import-open'.(count($activeStores)<1?' disabled':'').'>Impor Produk</button></div></div></form>';
 echo '<div class="muted small">Menampilkan '.count($finishedRows).' dari '.$totalRows.' produk. Filter diproses server agar UI tidak berat.</div>';
 echo '<div class="finished-table-wrap"><table class="finished-products-table"><thead><tr><th class="col-product">Produk</th><th class="col-sku">SKU</th><th class="col-price">Harga Jual Dapur</th><th class="col-stock">Stok Dapur</th><th class="col-source">Sumber</th><th class="col-action">Aksi</th></tr></thead><tbody>'; foreach($finishedRows as $r){ $stock=(float)($r['stock_qty']??0); $sourceLabel=(string)($r['store_name']??'Manual'); $isActive=!empty($r['is_active'])?'1':'0'; echo '<tr><td class="product-name-cell">'.e($r['name']).'</td><td>'.e($r['sku']).'</td><td>'.rupiah($r['transfer_price']).'</td><td>'.dec($stock).'</td><td>'.e($sourceLabel).'</td><td><div class="actions"><button type="button" class="btn light" data-finished-edit data-id="'.(int)$r['id'].'" data-code="'.e($r['code']??'').'" data-sku="'.e($r['sku']??'').'" data-name="'.e($r['name']??'').'" data-category="'.e($r['category']??'').'" data-unit="'.e($r['unit']??'pack').'" data-transfer-price="'.e((string)($r['transfer_price']??'0')).'" data-active="'.$isActive.'">Edit</button>'.(can_manage_finished_delete()?'<form method="post" onsubmit="return confirm(\'Hapus produk ini? Bila sudah ada histori, produk akan di-hide dari daftar.\')">'.csrf_field().'<input type="hidden" name="act" value="delete_finished"><input type="hidden" name="id" value="'.(int)$r['id'].'"><button class="btn danger">Hapus</button></form>':'').'</div></td></tr>'; } if(count($finishedRows)<1) echo '<tr><td colspan="6" class="muted">Tidak ada produk yang sesuai filter.</td></tr>'; echo '</tbody></table></div>';
 if($totalPages>1){ echo '<div class="pagination">'; if($curPage>1) echo '<a class="btn light" href="'.e($makeUrl(['p'=>$curPage-1])).'">‹ Sebelumnya</a>'; echo '<span class="page-current">Halaman '.$curPage.' / '.$totalPages.'</span>'; if($curPage<$totalPages) echo '<a class="btn light" href="'.e($makeUrl(['p'=>$curPage+1])).'">Berikutnya ›</a>'; echo '</div>'; }
...
elseif($page==='production'){
 if($_SERVER['REQUEST_METHOD']==='POST'){ $bom=one('SELECT * FROM bom_headers WHERE id=?',[(int)$_POST['bom_id']]); $qty=(float)postval('qty_produced','0'); if(!$bom||$qty<=0){flash('BOM/qty tidak valid','err');redirect('?page=production');} $factor=$qty/(float)$bom['yield_qty']; $bomItems=all('SELECT * FROM bom_items WHERE bom_id=?',[(int)$bom['id']]); $rawStock=stock_qty_map('raw',array_map(fn($x)=>(int)$x['raw_material_id'],$bomItems)); foreach($bomItems as $bi){$need=(float)$bi['qty']*$factor; if(($rawStock[(int)$bi['raw_material_id']]??0)+0.0001 < $need){flash('Stok bahan tidak cukup untuk produksi.','err');redirect('?page=production');}} $no=next_no('PRD','production_headers','production_no'); execq('INSERT INTO production_headers(production_no,production_date,bom_id,finished_product_id,qty_produced,status,notes,created_by,posted_at) VALUES(?,?,?,?,?,?,?,?,NOW())',[$no,postval('production_date',date('Y-m-d')),(int)$bom['id'],(int)$bom['finished_product_id'],$qty,'posted',postval('notes'),(int)($u['id']??0)]); $pid=(int)db()->lastInsertId(); foreach(all('SELECT bi.*, rm.last_cost FROM bom_items bi JOIN raw_materials rm ON rm.id=bi.raw_material_id WHERE bi.bom_id=?',[(int)$bom['id']]) as $bi){$used=(float)$bi['qty']*$factor; execq('INSERT INTO production_items(production_id,raw_material_id,qty_used,unit,unit_cost) VALUES(?,?,?,?,?)',[$pid,(int)$bi['raw_material_id'],$used,$bi['unit'],(float)$bi['last_cost']]); add_ledger('raw',(int)$bi['raw_material_id'],'production','production_headers',$pid,0,$used,(float)$bi['last_cost'],$no,(int)($u['id']??0));} add_ledger('finished',(int)$bom['finished_product_id'],'production','production_headers',$pid,$qty,0,null,$no,(int)($u['id']??0)); flash('Produksi diposting: '.$no); redirect('?page=production'); }
...
elseif($page==='stock'){
 h2('Stok Dapur');
 $rawRows=all('SELECT * FROM raw_materials ORDER BY name'); $rawMap=stock_qty_map('raw',array_column($rawRows,'id'));
 echo '<h3>Bahan Baku</h3><table><tr><th>Nama</th><th>Stok</th><th>Satuan</th></tr>'; foreach($rawRows as $r) echo '<tr><td>'.e($r['name']).'</td><td>'.dec($rawMap[(int)$r['id']]??0).'</td><td>'.e($r['unit']).'</td></tr>'; echo '</table>';
 $finishedRowsStock=all('SELECT * FROM finished_products ORDER BY name'); $finishedMap=stock_qty_map('finished',array_column($finishedRowsStock,'id'));
 echo '<h3>Barang Jadi</h3><table><tr><th>Nama</th><th>Stok</th><th>Satuan</th></tr>'; foreach($finishedRowsStock as $r) echo '<tr><td>'.e($r['name']).'</td><td>'.dec($finishedMap[(int)$r['id']]??0).'</td><td>'.e($r['unit']).'</td></tr>'; echo '</table>';
}
elseif($page==='stock_opname'){
 ...
 if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  $submittedIds=array_map('intval',array_keys($_POST['physical_qty']??[]));
  $systemStockMap=stock_qty_map($type,$submittedIds);
  $items=[];
  foreach(($_POST['physical_qty']??[]) as $id=>$physicalRaw){
   ...
   if(!$row) continue;
   $system=$systemStockMap[$id]??0; $diff=round($physical-$system,4);
 ...
 $table=$type==='raw'?'raw_materials':'finished_products'; $rows=all('SELECT * FROM '.$table.' WHERE is_active=1 ORDER BY name'); $stockMap=stock_qty_map($type,array_column($rows,'id'));
 ...
 foreach($rows as $r){ $st=$stockMap[(int)$r['id']]??0; echo '<tr><td>'.e($r['name']).'</td><td>'.dec($st).'</td><td><input name="physical_qty['.(int)$r['id'].']" type="number" step="0.0001" min="0" placeholder="kosongkan bila tidak diopname"></td><td>'.e($r['unit']).'</td></tr>'; }
...
elseif($page==='sales'){
 if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if($act==='resync_transfer'){
   ...
   $msg=$res['ok']?'Kirim API berhasil. Transfer menunggu konfirmasi toko.':'Kirim API gagal. '.$res['message'];
   ...
   flash($msg.' No: '.$h['sale_no'],$res['ok']?'ok':'err');
   redirect('?page=sales');
  }
  ...
  $postedIds=array_values(array_filter(array_map('intval',$_POST['finished_product_id']??[]),fn($v)=>$v>0));
  $stockMap=stock_qty_map('finished',$postedIds);
  $rows=[]; $total=0.0;
  ...
  try{
   ...
   execq('UPDATE kitchen_sales_headers SET total_amount=?, remote_response=? WHERE id=?',[$total,'Transfer lokal tersimpan. API belum dikirim agar UI Dapur tidak menunggu timeout koneksi toko. Klik Kirim API pada riwayat transfer.',$sid]);
   db()->commit();
  }catch(Throwable $e){ if(db()->inTransaction()) db()->rollBack(); flash('Gagal membuat transfer: '.$e->getMessage(),'err'); redirect('?page=sales'); }
  flash('Transfer lokal dibuat cepat. Klik Kirim API pada riwayat untuk mengirim ke toko. No: '.$no,'ok'); redirect('?page=sales');
 }
 h2('Penjualan / Distribusi ke Toko');
 echo '<div class="transfer-note">Posting transfer sekarang hanya menyimpan dan mengurangi stok lokal. Pengiriman API ke toko dilakukan dari tombol <b>Kirim API</b> pada riwayat, supaya UI Dapur tidak ikut menunggu timeout koneksi toko.</div>';
 $fps=all('SELECT fp.*, COALESCE(st.qty,0) stock_qty FROM finished_products fp LEFT JOIN (SELECT item_id, COALESCE(SUM(qty_in-qty_out),0) qty FROM stock_ledger WHERE item_type=\'finished\' GROUP BY item_id) st ON st.item_id=fp.id WHERE fp.is_active=1 ORDER BY fp.name');
 echo '<form method="post">'.csrf_field().'<input type="hidden" name="act" value="create_transfer"><div class="form-grid"><p><label>Tanggal<input name="sale_date" type="date" value="'.date('Y-m-d').'"></label></p><p><label>Toko Tujuan<select name="store_id">'; foreach(all('SELECT * FROM stores WHERE is_active=1 ORDER BY store_name') as $s) echo '<option value="'.(int)$s['id'].'">'.e($s['store_name']).'</option>'; echo '</select></label></p><p><label>Catatan<input name="notes"></label></p></div><table><tr><th>Produk</th><th>Qty</th><th>Harga Jual Dapur</th><th>Stok</th></tr>'; for($i=0;$i<6;$i++){ echo '<tr><td><select name="finished_product_id[]"><option value="">-</option>'; foreach($fps as $fp) echo '<option value="'.(int)$fp['id'].'">'.e($fp['name']).' — stok '.dec($fp['stock_qty']??0).'</option>'; echo '</select></td><td><input name="qty[]" type="number" step="0.0001"></td><td><input name="transfer_price[]" type="number" step="0.01" placeholder="otomatis dari produk"></td><td class="muted">lihat stok pada pilihan produk</td></tr>'; } echo '</table><p><button class="btn">Posting Transfer Lokal</button></p></form>';
 echo '<h3>Riwayat Transfer</h3><table><tr><th>No</th><th>Toko</th><th>Total</th><th>Status</th><th>Detail API</th><th>Aksi</th></tr>'; foreach(all('SELECT h.*,s.store_name FROM kitchen_sales_headers h LEFT JOIN stores s ON s.id=h.store_id ORDER BY h.id DESC LIMIT 30') as $r){ $detail=trim((string)($r['remote_response']??'')); $canSend=in_array((string)$r['status'],['posted','failed_sync'],true); echo '<tr><td>'.e($r['sale_no']).'</td><td>'.e($r['store_name']).'</td><td>'.rupiah($r['total_amount']).'</td><td><span class="badge">'.e($r['status']).'</span></td><td>'.($detail!==''?'<details><summary>Lihat</summary><pre class="log-pre">'.e(short_text($detail,1200)).'</pre></details>':'-').'</td><td>'.($canSend?'<form method="post">'.csrf_field().'<input type="hidden" name="act" value="resync_transfer"><input type="hidden" name="id" value="'.(int)$r['id'].'"><button class="btn light">Kirim API</button></form>':'-').'</td></tr>'; } echo '</table>';
}
