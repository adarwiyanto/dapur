...
elseif($page==='hope_connection'){
 ...
 foreach($conns as $c){ echo '<tr><td>'.e($c['connection_name']).'</td><td>'.e($c['remote_base_url']).'</td><td>'.e($c['access_scope']).'</td><td>'.status_badge2($c['status']).'</td><td>'.e($c['last_test_at']??'-').'<br><small>'.e($c['last_test_message']??'').'</small></td><td><div class="actions mini">'.((($c['connection_type']??'')==='outgoing')?'<form method="post" action="api_pairing_action.php">'.csrf_field().'<input type="hidden" name="return_page" value="hope_connection"><input type="hidden" name="act" value="test_connection"><input type="hidden" name="id" value="'.(int)$c['id'].'"><button class="btn light">Test Koneksi</button></form><form method="post" action="api_pairing_action.php">'.csrf_field().'<input type="hidden" name="return_page" value="hope_connection"><input type="hidden" name="act" value="test_hope_transfer"><input type="hidden" name="id" value="'.(int)$c['id'].'"><button class="btn light">Test Transfer Stok</button></form>':'<span class="muted small">Koneksi masuk; test dilakukan dari HOPe.</span>').'<form method="post" action="api_pairing_action.php" onsubmit="return confirm(&quot;Cabut koneksi HOPe ini?&quot;)">'.csrf_field().'<input type="hidden" name="return_page" value="hope_connection"><input type="hidden" name="act" value="revoke_connection"><input type="hidden" name="id" value="'.(int)$c['id'].'"><button class="btn danger">Hapus/Revoke</button></form></div></td></tr>'; }
...
elseif($page==='api_integrations'){
 ...
 $conns=table_exists('api_connections')?all("SELECT * FROM api_connections ORDER BY FIELD(status,'active','pending','revoked'), id DESC LIMIT 80"):[];
 ...
 foreach($conns as $c){ echo '<tr><td>'.e($c['remote_system_type']).'</td><td>'.e($c['connection_name']).'</td><td>'.e($c['remote_base_url']).'</td><td>'.e($c['access_scope']).'</td><td>'.status_badge2($c['status']).'</td><td>'.e($c['last_test_at']??'-').'<br><small>'.e($c['last_test_message']??'').'</small></td><td><div class="actions mini">'; if(($c['connection_type']??'')==='outgoing'){ echo '<form method="post" action="api_pairing_action.php">'.csrf_field().'<input type="hidden" name="return_page" value="api_integrations"><input type="hidden" name="act" value="test_connection"><input type="hidden" name="id" value="'.(int)$c['id'].'"><button class="btn light">Test Koneksi</button></form>'; if(($c['remote_system_type']??'')==='hope'){ echo '<form method="post" action="api_pairing_action.php">'.csrf_field().'<input type="hidden" name="return_page" value="api_integrations"><input type="hidden" name="act" value="test_hope_transfer"><input type="hidden" name="id" value="'.(int)$c['id'].'"><button class="btn light">Test Transfer Stok</button></form>'; }} else { echo '<span class="muted small">Koneksi masuk; test remote dilakukan dari aplikasi peminta.</span>'; } echo '<form method="post" action="api_pairing_action.php" onsubmit="return confirm(&quot;Cabut koneksi ini?&quot;)">'.csrf_field().'<input type="hidden" name="return_page" value="api_integrations"><input type="hidden" name="act" value="revoke_connection"><input type="hidden" name="id" value="'.(int)$c['id'].'"><button class="btn danger">Hapus/Revoke</button></form></div></td></tr>'; }
 if(!$conns) echo '<tr><td colspan="7" class="muted">Belum ada koneksi aplikasi.</td></tr>'; echo '</table>';
 $stores=table_exists('stores')?all('SELECT * FROM stores ORDER BY store_name'):[];
