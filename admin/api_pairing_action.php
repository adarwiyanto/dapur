...
try{
 ...
 if($act==='revoke_connection'){
   $id=(int)($_POST['id']??0); db()->prepare("UPDATE api_connections SET status='revoked',revoked_by=?,revoked_at=NOW(),last_test_status='revoked',last_test_message='Dicabut dari menu Dapur',updated_at=NOW() WHERE id=?")->execute([$uid,$id]);
   pairing_log_event(null,'api_connections','out','connection_revoked','Koneksi dicabut.',['connection_id'=>$id]); go_pair('Koneksi dicabut.');
 }
